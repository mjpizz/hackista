################################################################################
# Huge thanks to to Michael Lynn (pudquick) for the original implementation:
# https://gist.github.com/pudquick/4317095
# 
# This version fixes a few bugs I encountered and makes it easier to install
        ...
        if result:
            ...
            if os.path.exists(build_dir):
                ...
                for a_file in files:
                    ...
                    target = os.path.join(MODULES_DIR, a_file)
                    if os.path.exists(target):
                        _rm(target)
                    shutil.copyfile(os.path.join(build_dir, a_file), target)
                for a_dir in dirs:
                    print "* Installing module: %s ..." % a_dir
                    target = os.path.join(MODULES_DIR, a_dir)
                    if os.path.exists(target):
                        _rm(target)
                    shutil.copytree(os.path.join(build_dir, a_dir), target)
...
if not os.path.exists(TMP_DIR):
    try:
        ...
        # Fail silently, if we can't make the directory
        sys.exc_clear()
# Make sure MODULES_DIR exists before adding it to the paths
...
try:
    ...
    sys.exc_clear()
    _install_xmlrpclib(MODULES_DIR)
    import xmlrpclib
...
try:
    ...
    # Doesn't seem to be available - attempt to download it
    sys.exc_clear()
    _install_ConfigParser(MODULES_DIR)
    import xmlrpclib
try:
    ...
    sys.exc_clear()
    _install_setuptools(MODULES_DIR)
